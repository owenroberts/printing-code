...
void draw() {
  ...
  for (int x = 1; x < (width*height)/(box*box); x++) {
    if (counter < points.length) {
      ...
      RCommand.setSegmentLength(map(800 - points[counter][1], 0, 800, 0, 100));
      ...
      for (int k = 0; k < fontgroup.elements.length; k++) {
        ...
        for (int j = 0; j < fontpolygon.contours.length; j++) {
          ...
          for (int h = 0; h < index; h++) {
            ...
            stroke(black);
            vertex( ((catpoint.x + fontpoint.x)/2)*0.08, ((catpoint.y + fontpoint.y)/2)*0.08 + box/2);
          }
          endShape();
  ...
  endRecord();
// saveFrame("imgs/"+ getClass().getName() + month()+ "_" + day() + "_" + hour() + "_" + minute() +"line-######.tif");
}
